...
### v0.5 — 图片存储迁移（2026-05-13）

#### 5.0.1 图片存储迁移到 MySQL
- `schema.sql` uploads 表新增 `data LONGBLOB` 列，用于存储图片二进制数据
- `admin.py` `upload_image()` 改为读取文件二进制内容并写入数据库，返回 `/media/<id>` 格式 URL
- `main.py` `serve_upload()` 改为从数据库查询图片数据并返回，带 30 天缓存头
- 兼容旧数据：非数字 ID 的请求回退到文件系统读取
